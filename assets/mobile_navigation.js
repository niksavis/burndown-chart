...
function handleSwipeGesture() {
    ...
    if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > mobileNavState.swipeThreshold) {
        const currentIndex = mobileTabsConfig.findIndex(tab => tab.id === mobileNavState.currentTab);
...
function switchToTab(tabId) {
    ...
    // Trigger Dash callback (if available)
    if (window.dash_clientside) {
        // Use Dash's clientside callback system
        const tabElement = document.querySelector(`#chart-tabs [data-value="${tabId}"]`);
        if (tabElement) {
            tabElement.click();
        }
    }
