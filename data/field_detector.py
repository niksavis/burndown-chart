...
def _detect_work_started_date_field(
    issues: List[Dict], field_defs: Dict[str, Dict]
) -> Optional[str]:
    """Detect work started date field for Flow Time.

    Heuristics:
    - Field name contains: "start", "began", "in progress", "active"
    - Field type: datetime
    - Has date values in the past
    """
    candidates = {}
...
def _detect_work_completed_date_field(
    issues: List[Dict], field_defs: Dict[str, Dict]
) -> Optional[str]:
    """Detect work completed date field for Flow Time.

    Heuristics:
    - Field name contains: "complete", "finish", "done", "closed"
    - Field type: datetime
    - Has date values in the past
    """
    candidates = {}
...
def detect_fields_from_issues(
    ...
    # 10. Detect work started date field (for Flow Time)
    # Fallback: Use status transitions to 'In Progress' from changelog
    ...
    if work_started_date:
        ...
        logger.info(
            ...
            "Fallback: Will use status transitions to 'In Progress' from changelog"
        )

    # 11. Detect work completed date field (for Flow Time)
    # Fallback: Use resolutiondate (standard field)
    work_completed_date = _detect_work_completed_date_field(sampled_issues, field_defs)
    if work_completed_date:
        ...
        logger.info(
            "[FieldDetector] No work completed date field found. "
