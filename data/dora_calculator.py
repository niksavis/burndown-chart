...
DEPLOYMENT_FREQUENCY_TIERS = {
    "Elite": {"min": 999999, "unit": "per_day", "color": "green"},  # On-demand (multiple per day)
    "High": {"min": 1, "max": 999999, "unit": "per_week", "color": "yellow"},  # Weekly
    "Medium": {"min": 1, "max": 999999, "unit": "per_month", "color": "orange"},  # Monthly
    "Low": {"min": 0, "max": 1, "unit": "per_month", "color": "red"},  # Less than monthly
}

LEAD_TIME_TIERS = {
    "Elite": {"max": 1, "unit": "days", "color": "green"},  # Less than one day
    "High": {"min": 1, "max": 7, "unit": "days", "color": "yellow"},  # One day to one week
    "Medium": {"min": 7, "max": 30, "unit": "days", "color": "orange"},  # One week to one month
...
MTTR_TIERS = {
    ...
    "High": {"min": 1, "max": 24, "unit": "hours", "color": "yellow"},  # One hour to one day
    "Medium": {"min": 24, "max": 168, "unit": "hours", "color": "orange"},  # One day to one week
...
def _determine_performance_tier(value: float, tier_config: Dict[str, Any]) -> Dict[str, str]:
...
def calculate_deployment_frequency(
    ...
    tier_info = _determine_performance_tier(deployments_per_month, DEPLOYMENT_FREQUENCY_TIERS)
...
def calculate_lead_time_for_changes(
    ...
    for issue in issues:
        ...
        if lead_time_days < 0:
            ...
            excluded_count += 1
            logger.warning(f"Issue {issue.get('key')}: deployment date before commit date")
            continue
...
def calculate_mean_time_to_recovery(
    ...
    for issue in issues:
        ...
        if recovery_hours < 0:
            ...
            excluded_count += 1
            logger.warning(f"Issue {issue.get('key')}: resolved date before detected date")
            continue
