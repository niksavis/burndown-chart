...
- [ ] T051 [US3] Verify forecast data is being persisted in weekly snapshots (should already be working from US1 T023)
- [ ] T052 [US3] Test loading historical snapshot from `metrics_snapshots.json` and confirm forecast field is present
- [ ] T053 [US3] Ensure `get_metrics_snapshot_for_week(week_key)` returns forecast data when loading historical weeks

### Historical Display for US3

- [ ] T054 [US3] Verify `create_metric_card()` uses forecast data from snapshot when displaying historical weeks (no code change needed if US1 implemented correctly)
- [ ] T055 [US3] Ensure performance badges reflect achieved performance tier, not forecast-based tier, for historical weeks
- [ ] T056 [US3] Add historical context: when viewing past week, forecast represents "what was predicted at that time"

### Verification for US3

- [ ] T057 [US3] Manual test: Select Week 2025-W45 with 18 items completed, verify shows "Forecast: ~13 items/week" and "↗ +38% above forecast"
- [ ] T058 [US3] Verify performance badge shows "Elite" based on achieved 18 items, not forecast of 13
- [ ] T059 [US3] Test historical week with underperformance, verify downward trend arrow and percentage appear correctly
...
- [ ] T060 [US4] Verify `calculate_flow_load_range()` is being called for Flow Load metric in `save_metrics_snapshot()` (should already be working from US1 T025)
- [ ] T061 [US4] Enhance Flow Load trend calculation to use range-based comparison: above upper bound, within range, below lower bound
- [ ] T062 [US4] Map range status to appropriate styling: text-warning for too high, text-success for in range, text-info for too low

### Flow Load Display for US4

- [ ] T063 [US4] Verify `create_forecast_section()` displays Flow Load range format correctly (should already be working from US1 T030)
- [ ] T064 [US4] Update Flow Load status text: "Above normal range" vs "Within normal range ✓" vs "Below normal range"
- [ ] T065 [US4] Apply bidirectional styling: both ↗ and ↘ can be bad for Flow Load (too high or too low)

### Verification for US4

- [ ] T066 [US4] Manual test: Set WIP=24 with range 12-18, verify "Forecast: ~15 items (12-18)" and "↗ +60% above normal" with warning color
- [ ] T067 [US4] Manual test: Set WIP=14 with range 12-18, verify "→ Within normal range ✓" with success color
- [ ] T068 [US4] Manual test: Set WIP=9 with range 12-18, verify "↘ -40% below normal" with info color
...
- [ ] T069 [US5] Verify `calculate_forecast()` handles 2-3 week data with equal weighting (should already be working from T005 foundation)
- [ ] T070 [US5] Test forecast returns None when <2 weeks of data available
- [ ] T071 [US5] Verify forecast confidence field is set to "building" for <4 weeks, "established" for 4 weeks

### Baseline Display for US5

- [ ] T072 [US5] Update forecast text to show "(based on N weeks)" when weeks_available < 4 in `create_forecast_section()`
- [ ] T073 [US5] Display "🆕 Building baseline..." message when `forecast_data["confidence"] == "building"` (should already be working from US1 T031)
- [ ] T074 [US5] Handle case when forecast is None (insufficient data): display "Insufficient data for forecast" message

### Verification for US5

- [ ] T075 [US5] Test with 2 weeks of data: verify "Forecast: ~11 items (based on 2 weeks)" and "🆕 Building baseline..." appear
- [ ] T076 [US5] Test with 3 weeks of data: verify equal weighting used and baseline message appears
- [ ] T077 [US5] Test with 1 week of data: verify "Insufficient data for forecast" message appears instead of forecast
