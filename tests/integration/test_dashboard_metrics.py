...
class TestDashboardMetricsConsistency(unittest.TestCase):
    ...
    def test_completion_dates_consistency(self):
        """Test that forecast completion dates are consistent across different calculations."""
        # Generate forecast directly
        forecast = generate_weekly_forecast(
            self.statistics_data, self.pert_factor, forecast_weeks=8
        )

        # Get forecast through visualization preparation
        viz_data = prepare_visualization_data(
            self.statistics_data, self.total_items, self.total_points, self.pert_factor
        )

        # Calculate completion date from direct forecast (where most_likely values reach total)
        direct_completion_date = None
        if "dates" in forecast["items"] and "most_likely" in forecast["items"]:
            most_likely_items = forecast["items"]["most_likely"]
            dates = forecast["items"]["dates"]

            # Try to parse dates (could be strings or datetime objects)
            if len(dates) > 0 and len(most_likely_items) > 0:
                for i, value in enumerate(most_likely_items):
                    if isinstance(dates[i], str):
                        try:
                            # Parse date if it's a string
                            direct_completion_date = datetime.strptime(
                                dates[i], "%b %d"
                            )
                            direct_completion_date = direct_completion_date.replace(
                                year=2025
                            )  # Assume current year
                            break
                        except ValueError:
                        ...
                        direct_completion_date = dates[i]
                        break

        # Calculate completion date from visualization forecast using items_forecasts
        viz_completion_date = None
        if "items_forecasts" in viz_data and "avg" in viz_data["items_forecasts"]:
            dates, values = viz_data["items_forecasts"]["avg"]
            if len(dates) > 0 and len(values) > 0:
                for i, value in enumerate(values):
                    if value <= 0:
                        viz_completion_date = dates[i]
                        break

                # If we didn't find a zero value, take the last date
                if viz_completion_date is None and len(dates) > 0:
                    viz_completion_date = dates[-1]

        # If both dates were found, they should be close to each other
        # We allow some small differences due to different calculation paths
        if direct_completion_date is not None and viz_completion_date is not None:
            # Convert to datetime if needed
            if isinstance(direct_completion_date, str):
                try:
                    direct_completion_date = datetime.strptime(
                        direct_completion_date, "%b %d"
                    )
                    direct_completion_date = direct_completion_date.replace(
                        year=2025
                    )  # Assume current year
                    ...
                    try:
                        direct_completion_date = datetime.strptime(
                            direct_completion_date, "%Y-%m-%d"
                        )
                    except ValueError:
                        self.skipTest("Could not parse direct completion date format")

            # Calculate difference in days
            date_diff = abs((direct_completion_date - viz_completion_date).days)
            self.assertLessEqual(
                date_diff, 30
            )  # Allow up to 30 days difference due to different calculation methods
