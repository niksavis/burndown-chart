...
class TestProfileCascadeDeletion:
    ...
    def temp_profiles_with_data(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            kafka_main_dir = temp_profiles_dir / "kafka" / "queries" / "main"
            ...
            kafka_bugs_dir = temp_profiles_dir / "kafka" / "queries" / "bugs"
            ...
            spark_12w_dir = temp_profiles_dir / "spark" / "queries" / "12w"
            ...
            spark_6w_dir = temp_profiles_dir / "spark" / "queries" / "6w"
    ...
    def test_delete_profile_cascades_to_all_queries_and_data(
        ...
        delete_profile("spark")

        # Verify spark profile directory completely removed
        spark_dir = temp_profiles_with_data / "spark"
        ...
        profiles_file = temp_profiles_with_data / "profiles.json"
        with open(profiles_file, "r") as f:
            metadata = json.load(f)

        assert "spark" not in metadata["profiles"]
        assert "kafka" in metadata["profiles"]  # Other profile untouched

    def test_delete_profile_prevents_deleting_active_profile(
        self, temp_profiles_with_data
    ):
        """Verify cannot delete active profile."""
        with pytest.raises(ValueError) as exc_info:
            delete_profile("kafka")  # Active profile

        assert "Cannot delete active profile" in str(exc_info.value)
        assert "Switch to another profile first" in str(exc_info.value)

        # Verify kafka profile still exists
        kafka_dir = temp_profiles_with_data / "kafka"
    ...
    def test_delete_profile_continues_on_query_deletion_errors(
        ...
        with open(profiles_file, "r") as f:
            metadata = json.load(f)
        metadata["active_profile_id"] = "spark"
        with open(profiles_file, "w") as f:
            json.dump(metadata, f)
    ...
    def test_delete_profile_updates_profiles_registry_atomically(
        ...
        assert "spark" in initial_metadata["profiles"]
        assert "kafka" in initial_metadata["profiles"]
        ...
        assert "spark" not in updated_metadata["profiles"]
        assert "kafka" in updated_metadata["profiles"]
...
class TestCascadeDeletionEdgeCases:
    ...
    def temp_profile_with_many_queries(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            profiles_data = {
                ...
                "profiles": {
                    "default": {"id": "default", "name": "Default", "queries": []},
                    "bulk-test": {
                        "id": "bulk-test",
                        "name": "Bulk Test",
            ...
            for i in range(1, 21):
                query_dir = temp_profiles_dir / "bulk-test" / "queries" / f"query-{i}"
    ...
    def test_delete_profile_handles_many_queries(self, temp_profile_with_many_queries):
        ...
        delete_profile("bulk-test")

        # Verify entire profile removed
        bulk_test_dir = temp_profile_with_many_queries / "bulk-test"
