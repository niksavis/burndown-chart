...
Example:
    python split-excalidraw-library.py \
...
def split_library(library_dir: str | Path) -> None:
    """
    ...
    if not library_dir_path.exists():
        print(f"Error: Directory not found: {library_dir_path}")
        sys.exit(1)

    if not library_dir_path.is_dir():
        print(f"Error: Path is not a directory: {library_dir_path}")
        sys.exit(1)

    # Find the library file
    library_path = find_library_file(library_dir_path)
    ...
    icons_dir = library_dir_path / "icons"
    icons_dir.mkdir(exist_ok=True)
    print(f"Output directory: {library_dir_path}")
    ...
    reference_path = library_dir_path / "reference.md"
    with open(reference_path, "w", encoding="utf-8") as f:
        f.write(f"# {library_name} Reference\n\n")
        f.write(
            f"This directory contains {len(icon_list)} icons extracted from "
            f"`{library_path.name}`.\n\n"
        )
        ...
        f.write("\n## Usage\n\n")
        f.write(
            "Each icon JSON file contains the complete `elements` array needed "
            "to render that icon in Excalidraw.\n"
        )
        f.write(
            "You can copy the elements from these files into your Excalidraw "
            "diagrams.\n"
        )
